...
type CorporateEventsQuoteFormProps = {
  cityName?: string;
};
...
export default function CorporateEventsQuoteForm({
  cityName,
}: CorporateEventsQuoteFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = `Corporate & Office Event Decoration Quote Request
...
Message: ${formData.get("message")}`;
    ...
    trackPopupSubmit("corporate_events_page");
    trackBookingClick("corporate_events_page");
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      ...
      <select name="budget" required className={selectClassName}>
        ...
        <option>Starting from Rs. 4,999</option>
        <option>Rs. 10,000 - Rs. 20,000</option>
        <option>Rs. 20,000 - Rs. 50,000</option>
        <option>Premium Corporate Setup Rs. 50,000+</option>
      </select>
