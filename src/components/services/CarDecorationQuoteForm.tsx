...
type CarDecorationQuoteFormProps = {
  cityName?: string;
};
...
export default function CarDecorationQuoteForm({
  cityName,
}: CarDecorationQuoteFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = `Car Boot & Car Decoration Quote Request
...
Message: ${formData.get("message")}`;
    ...
    trackPopupSubmit("car_decoration_page");
    trackBookingClick("car_decoration_page");
    trackWhatsAppClick("car_decoration_quote");
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      ...
      <select name="budget" required className={selectClassName}>
        ...
        <option>Luxury Car Surprise Rs. 20,000+</option>
      </select>
      ...
      <div className="flex flex-col gap-3 sm:col-span-2 sm:flex-row">
        ...
        <a
          href="https://wa.me/919602060414?text=Hi%2C%20I%20want%20car%20boot%20and%20car%20decoration%20details"
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => trackWhatsAppClick("car_decoration_contact")}
