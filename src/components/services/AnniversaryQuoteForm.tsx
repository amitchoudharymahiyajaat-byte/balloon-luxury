...
type AnniversaryQuoteFormProps = {
  cityName?: string;
};
...
export default function AnniversaryQuoteForm({
  cityName,
}: AnniversaryQuoteFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = `Anniversary & Romantic Decoration Quote Request
...
Message: ${formData.get("message")}`;
    ...
    trackPopupSubmit("anniversary_decoration_page");
    trackBookingClick("anniversary_decoration_page");
    trackWhatsAppClick("anniversary_decoration_quote");
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
      ...
      <select name="budget" required className={selectClassName}>
        ...
        <option>Luxury Romantic Setup Rs. 20,000+</option>
      </select>
      ...
      <div className="flex flex-col gap-3 sm:col-span-2 sm:flex-row">
        ...
        <a
          href="https://wa.me/919602060414?text=Hi%2C%20I%20want%20anniversary%20and%20romantic%20decoration%20details"
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => trackWhatsAppClick("anniversary_decoration_contact")}
